...
"""
    Train models using the original texts.
"""
import numpy as np
import pandas as pd
import os
...
tf.flags.DEFINE_boolean("adv", False, "If use adversarial training method.")
...
def train(x_train, y_train, x_dev, y_dev, embedding_matrix, num_classes):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs_org_%s" % FLAGS.nn_type, timestamp))
            print("Writing to {}\n".format(out_dir))
            ...
            for batch in batches:
                ...
                batches_dev = data_helpers.batch_iter(list(zip(x_dev, y_dev)), FLAGS.batch_size*4, 1)
                time_str = datetime.datetime.now().isoformat()
                ...
                if current_step % FLAGS.evaluate_every == 0:
                    ...
                    if accuracy_mean > best_eval_accuracy:
                        if FLAGS.adv == True or step > 300:
...
def main(argv=None):
    path = FLAGS.data
    train_x, train_y, test_x, test_y, embedding_matrix = preprocess(path)
    ...
    train(train_x, train_y, test_x, test_y, embedding_matrix, num_classes)
